...
*MSNoise* is especially useful for large datasets and continuous monitoring. Configuration and the state of a project is managed by sqlite or mysql database. A project can be configured via web interface, commands are issued via command line interface. Velocity variations are determined with the Moving Window Cross Spectral technique (MWCS).
*MIIC* is another monitoring library using the time-domain stretching technique.

*Yam*, contrary to MSNoise, is designed for off-line usage, but also includes capabilities to reprocess continuously growing data. Yam does not rely onto a database, but rather checks on the fly which results already exist and which results have still to be calculated.
Cross-correlations are written to HDF5 files via the ObsPy plugin obspyh5. Thus, correlation data can be easily accessed with ObsPy's :func:`~obspy.core.stream.read` function after the calculation. It follows a similar processing flow as MSNoise, but it uses the stretching library from MIIC. (It is of course feasible to implement MWCS.)
One of its strong points is the configuration declared in a simple, but heavily commented JSON file.
It is possible to declare similar configurations. A possible use case is the reprocessing of the whole dataset in a different frequency band.
...
This means data was correlated (``c``) with configuration ``1``, each two days ``2d`` are stacked (``s``) and finally data was stretched (``t``) using the stretching configuration ``wow``. The configuration of the keys are defined in the configuration file.
(These ids may not contain ``_``, because ``_`` is used to sparate the different processing steps.)
The ``s`` key is special, because it can describe the stacking procedure directly:
For example, ``s5d`` stacks correlations of 5 days, ``s2h`` of 2 hours, ``s5dm2.5d`` is a 2.5 day moving (``m``) stack over 5 days, with ``d`` corresponding to days and ``h`` corresponding to hours.
...
An advanced tutorial can be studied `here <http://nbviewer.jupyter.org/github/trichter/notebooks/blob/master/notebooks/velocity_variations_patcx/velocity_variations_patcx.ipynb>`_.
...
Use ObsPy's :func:`~obspy.core.stream.read` to read correlations and stacks and `~yam.commands.read_dicts()` to read stretching results.
...
It follows a table with links to functions which consume the options. All config options should be documented inside these functions.
...
More information about the different subcommands of yam can be found in the corresponding functions in `~yam.commands` module.
"""
